...
/**
 * Brand marks for the project's social accounts.
 *
 * These are code-managed per CLAUDE.md (social icons are an icon component;
 * only the destination URLs are configuration). Lucide carries no brand
 * glyphs, so these are inline SVG paths rather than a second icon library.
 */
const NETWORKS = [
  {
    name: "Facebook",
    href: "https://facebook.com/bhashasetu",
    brand: "#1877F2",
  ...
  {
    name: "Instagram",
    href: "https://instagram.com/bhashasetu",
    brand: "#E4405F",
  ...
  {
    name: "YouTube",
    href: "https://youtube.com/bhashasetu",
    brand: "#FF0000",
...
export function SocialLinks({
  ...
  className?: string;
  size?: number;
  /** "brand" uses each network's colour; "mono" inherits currentColor. */
  tone?: "brand" | "mono";
}) {
  return (
    <div className={className ? `social-links ${className}` : "social-links"}>
      {NETWORKS.map((n) => {
        const style =
          tone === "brand"
            ? ({ "--brand": n.brand } as CSSProperties)
            : undefined;
        return (
          <a
            key={n.name}
